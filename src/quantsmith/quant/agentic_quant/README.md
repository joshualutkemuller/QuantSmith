...
See [`Quant Skills Library`](../../../../agents/quant_analyst/Quant%20Skills%20Library) for the system prompt
...
from quantsmith.quant.agentic_quant import ModelTrainingAgent
...
3. Run the agentic workflow to see the autonomous pipeline in action:
   ...
   ```bash
   python -m quantsmith.quant.agentic_quant.main
   ```
   ...
   without invoking the CLI entry point, import from
   `quantsmith.quant.agentic_quant` directly.
   ...
   ```python
   from quantsmith.quant.agentic_quant import build_pipeline, run_workflow
...
```python
from quantsmith.quant.agentic_quant import YahooFinanceDataAgent, run_workflow
...
[`cli/sp500.py`](cli/sp500.py).  The agent defaults to the "stooq" data source
...
```python
from quantsmith.quant.agentic_quant import PandasDataReaderDataAgent, build_pipeline, get_sp500_tickers
...
Running the packaged `quantsmith-sp500` command wraps the same setup in a CLI. Any
...
```python
from quantsmith.quant.agentic_quant import build_pipeline
...
```bash
quantsmith-rebalancer --rebalance-frequencies 5,21,63 --transaction-cost 0.00025
```
...
```python
from quantsmith.quant.agentic_quant import run_sp500_workflow
...
[`agents.py`](agents.py) to plug in live data,
