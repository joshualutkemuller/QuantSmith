...
| `adapters/dashboard_render/` provider implementations | Executable providers behind the contract. Shipped: `scaffold_react` (stdlib) and `write_xlsx` (openpyxl, lazy) — spec `specs/0017-dashboard-render-adapters/`. Remaining: a `powerbi_publish` provider and a hosted-deploy step | P2 | in-progress |
