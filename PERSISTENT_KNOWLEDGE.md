...
| **`0056` market research** | *Draft — reference implementation only.* Real spec (MCP namespace, entitlement enforcement, email connector, quarantine detection) not built; a small fictional-content-only `research/` store + terminal view exists to prove the schema. `0058` enforces its `access_level` field the same way it does workflow memory's. See [Market research & color](#market-research--color-spec-0056--reference-only) above. |
