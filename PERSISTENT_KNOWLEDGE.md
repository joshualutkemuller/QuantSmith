...
| Spec | `0002` (store), `0048` (read path), `0049` (write path) | `0056` (market research, **Draft**, reference implementation only) |
| Runtime | [`src/quantsmith/pipelines/workflow_memory.py`](src/quantsmith/pipelines/workflow_memory.py) | [`src/quantsmith/knowledge_console/research.py`](src/quantsmith/knowledge_console/research.py) (reads `research/` only — no MCP interface, entitlement enforcement, or email connector; see [Market research & color](#market-research--color-spec-0056--reference-only) below) |
...
| **`0057` front end** | **Done.** Reference console (`web/`) and Bloomberg-terminal app (`apps/knowledge-console/`), both read-only, both sourced from the same Python view-model. See [The front end](#the-front-end--knowledge-console--terminal-spec-0057) above. Per-viewer access-level enforcement (today: display-only) and the approval *action* (write-back UI) are named, deferred gaps, not oversights. |
| **`0056` market research** | *Draft — reference implementation only.* Real spec (MCP namespace, entitlement enforcement, email connector, quarantine detection) not built; a small fictional-content-only `research/` store + terminal view exists to prove the schema. See [Market research & color](#market-research--color-spec-0056--reference-only) above. |
| **`0052`–`0054` MCP servers** | *Planned, not written.* Exposing workflow memory and the knowledge base to a team over MCP: a resources server, a memory-graph server with `as_of` honoring the point-in-time rule, a RAG server with per-access-tier indexes. See item 17. |
| **`access_level` enforcement** | Parsed and stored on every workflow-memory record and every research item; **neither `query()` nor the front ends filter on it by viewer yet** — both display it, neither enforces it. For a firm with real information barriers this is a blocker before any shared deployment, not a nice-to-have. Belongs with the MCP work, where caller clearance has to be a parameter rather than an assumption. |
