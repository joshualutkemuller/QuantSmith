...
| T-012 | Wire catalogs and docs: `specs/README.md`, root `README.md` runtime table, `src/quantsmith/pipelines/README.md`, `instructions/workflow_memory.md` (author + pseudonymity, `superseded_by`, `coexists`, list-form `evidence`, the type-based PIT rule), `agents/knowledge/*` (point at the runtime). | REQ-001 | done | `doc-counts`, `spec-index`, and `readme-sync` all enforce parts of this. The standard is `0002`'s and gains fields here — update it rather than letting the runtime and the standard diverge. |
