...
| BI-tool profiles: `tooling/looker`, `tooling/qlik`, `tooling/superset`, `tooling/streamlit_dash` | Thin profiles that render the shared dashboard spec from `dashboard_design` (spec `0014` track). Power BI shipped first as `specs/0015-powerbi-dashboard-profile/` (`render_powerbi` + the shared `DashboardSpec`; also repaired the dead `powerbi.py` contract). These follow the same pattern | P2 | in-progress |
