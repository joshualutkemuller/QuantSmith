...
- **Quant research:** `0001` signal → `0006` forecast (`0041` ranking-loss variant) → `0007` portfolio → `0012` execution → `0038` factor risk (decomposition, attribution, stress) → `0044` backtest (net of costs, probabilistic Sharpe, no look-ahead by construction). The chain now ends in a measured result rather than a design.
