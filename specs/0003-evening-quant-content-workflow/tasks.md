...
| --- | --- | --- | --- | --- |
| T-001 | Add `configs/evening_quant_content.yml`. | REQ-001, REQ-006, NFR-004 | done | Config-driven schedule, limits, topics, review, memory, delivery. |
| T-002 | Promote the handoff into this spec package. | REQ-001, REQ-002 | done | `spec.md`, `plan.md`, `tasks.md`. |
| T-003 | Add `agents/content/README.md` group workflow. | REQ-001, REQ-008 | done | Content group mini-map. |
...
| T-005 | Add draft-pack template. | REQ-003, NFR-002 | done | `templates/docs/evening_quant_draft_pack.md`. |
...
| T-011 | Add runnable workflow executor. | REQ-002, REQ-003, REQ-008 | todo | Future runtime implementation. |
| T-012 | Add scheduler deployment profile. | REQ-008 | todo | Future adapter-specific deployment. |
...
| AC-004 | claim review source-note rule | done (contract), todo (runtime) |
...
- Implement a runtime executor that transforms context into a draft pack.
- Add source-provider profiles used by the nightly market context stage.
- Add a scheduler deployment artifact when a real delivery target is selected.